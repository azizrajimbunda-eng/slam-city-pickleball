...
- **Paused for playtesting on purpose.** User + colleague are playing full games on current `main` before any more changes land — "better in a quick test" and "better after 20 real rallies" can diverge, so don't start the next feature unprompted. Resume when new feedback comes in, or when the user explicitly says go ahead on tiers.
...
- Next up when resumed (user-agreed order): tiers/progression scoping (user open to it now that tuning holds across two testers — the old "no difficulty modes" removal was because mechanics weren't tuned yet); multiplayer (online + co-op — connection architecture question parked: P2P room codes vs relay server).
