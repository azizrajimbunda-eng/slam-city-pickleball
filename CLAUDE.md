...
- **Next up (user-chosen 2026-07-15, after the kitchen-line fix): (1) more AI court-sense, (2) between-point flow.** Then the older roadmap: tiers/progression scoping (user open to it now that tuning holds across testers — the old "no difficulty modes" removal was because mechanics weren't tuned yet); multiplayer (online + co-op — connection architecture question parked: P2P room codes vs relay server).
  - Known court-sense lead, found during the kitchen-line probe but deliberately not fixed in that batch: after the return, the returning team parks ~9.7ft back because the `adv` ramp in `updatePlayers()` (`clamp((shotCount-startShot+1)/2,0,1)`) sits at 0.5 for a beat — the classic no-man's-land error; real players run *through* the transition zone to the line. Also unexplored: partner shifting/covering, poaching.
